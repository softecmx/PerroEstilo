...
public class CrearDiseniosFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        root = inflater.inflate(R.layout.fragment_agregar_disenios, container, false);
        cargar();
        iniciaFirebase();
        return root;
    }
    public void cargar(){
        btnAgregar = root.findViewById(R.id.btnFrameAgregaDisenioAceptar);
        etNombre = root.findViewById(R.id.etFrameAgregaDisenioNom);
        ivPopImg = root.findViewById(R.id.imgFrameAgregaDisenioFoto);
        fbPopFoto = root.findViewById(R.id.fBtnFrameAgregaDisenioFoto);
...
;            }
    ...
    public void valida(){
        if (img.equals("")||etNombre.getText().toString().equals("")){
            CustomToast.mostarToast("Faltan datos",2,false,root,getLayoutInflater(),getContext());
        }else{
            Disenios dis = new Disenios();
            dis.setDisenio(etNombre.getText().toString());
            dis.setImagen(img);
            dis.setEstadoLogico("1");
            dis.setIdModelo(UUID.randomUUID().toString());
            databaseReference.child("Disenios").child(dis.getIdModelo()).setValue(dis);
