...
import com.programacion.perroestilocliente.ui.cliente.perfil.ListAdapter;
...
public class PerfilAdministradorFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        root = inflater.inflate(R.layout.fragment_perfil_administrador, container, false);
        FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();
        firebaseDatabase = FirebaseDatabase.getInstance();
        databaseReference = firebaseDatabase.getReference();
        storageReference = FirebaseStorage.getInstance().getReference("Perfiles");

        cImgFoto = root.findViewById(R.id.imgPerfilFotoAdmin);

        fBtnFoto = root.findViewById(R.id.fBtnPerfilFotoAdmin);
        this.listView = root.findViewById(R.id.listViewCuentaAdmin);
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (snapshot.exists()) {
                            for (DataSnapshot objSnapshot : snapshot.getChildren()) {
                                 usuario = objSnapshot.getValue(Administrador.class);
            ...
            if (networkInfo != null && networkInfo.isConnected()) {
                ...
                ref.putFile(photoURI).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                    ...
                    public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                        ...
                        mapear.put("Usuarios/Tienda/" + usuario.getIdUsuario() + "/fotoPerfil", img);
                        databaseReference.updateChildren(mapear);
                        mostarToast("En unos momentos se visualizaran los cambios...", 0, true);
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
