...
public class ListAdapterTicket extends ArrayAdapter<DetOrdenProductos> {

    private Activity activity;
    private ArrayList<DetOrdenProductos> arrayList;
    FirebaseDatabase firebaseDatabase = FirebaseDatabase.getInstance();
    DatabaseReference databaseReference = firebaseDatabase.getReference();
    StorageReference storageReference = FirebaseStorage.getInstance().getReference("Productos");
    FirebaseUser user = FirebaseAuth.getInstance().getCurrentUser();

    public ListAdapterTicket(Activity activity, ArrayList<DetOrdenProductos> arrayList) {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        txtCarritoPrecioUnitario.setText("$" + arrayList.get(position).getPrecioUnitario());
        float sub=(arrayList.get(position).getPrecioUnitario() * arrayList.get(position).getCantidad());
        txtCarritoSubtotal.setText("Subtotal: $" +sub);
        ...
        databaseReference.child("Productos").orderByChild("idProducto").equalTo(arrayList.get(position).getIdProducto()).addValueEventListener(new ValueEventListener() {
            @Override
